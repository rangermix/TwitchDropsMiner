...
(async()=>{
 ...
 panel.data.session={state:'ready',paired:false,expires_at:4600};
 panel.render();
 assert.equal(doc.getElementById('session-import-pair').disabled,false);
 assert.equal(doc.getElementById('session-import-revoke').disabled,true);
 let downloaded;
 panel.download=data=>{downloaded=data};
 reply={ok:true,json:async()=>({version:1,credential:'test-only-private',endpoint:'https://tdm.test/api/session/renew',user_id:42})};
 await panel.manage('pair');
 assert.equal(downloaded.credential,'test-only-private');
 assert.equal(calls.at(-1).url,'/api/session/pair');
 assert.equal(calls.at(-1).options.headers['X-TDM-Request'],'1');
 assert.equal(doc.getElementById('session-import-revoke').disabled,false);
 for (const el of elements.values()) assert.equal(el.textContent.includes('test-only-private'),false);
 reply={ok:true,json:async()=>({success:true,session:{state:'ready',paired:false,expires_at:4600}})};
 await panel.manage('revoke');
 assert.equal(calls.at(-1).url,'/api/session/revoke');
 assert.equal(doc.getElementById('session-import-revoke').disabled,true);
 reply={ok:false,json:async()=>({detail:'private error must not echo'})};
 await panel.manage('pair');
 assert.equal(doc.getElementById('session-import-status').textContent,'Failed (connection_failed)');
