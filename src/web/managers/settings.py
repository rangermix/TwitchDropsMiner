...
class SettingsManager:
    ...
    def update_settings(self, settings_data: dict[str, Any]):
        ...
        should_trigger_update |= self.check_and_update_setting("games_to_watch", settings_data["games_to_watch"], True)
        should_trigger_update |= self.check_and_update_setting("dark_mode", settings_data["dark_mode"])
        should_trigger_update |= self.check_and_update_setting("language", settings_data["language"], False, self._set_language)
        should_trigger_update |= self.check_and_update_setting("connection_quality", settings_data["connection_quality"])
        should_trigger_update |= self.check_and_update_setting("proxy", URL(settings_data["proxy"].strip()), True, lambda proxy: self._log_change("Proxy cleared") if proxy == URL() else None)
        should_trigger_update |= self.check_and_update_setting("minimum_refresh_interval_minutes", settings_data["minimum_refresh_interval_minutes"])
        should_trigger_update |= self.check_and_update_setting("inventory_filters", settings_data["inventory_filters"])
        should_trigger_update |= self.check_and_update_setting("mining_benefits", settings_data["mining_benefits"], True)
    ...
    def check_and_update_setting(self, key: str, new_value: Any, should_trigger_update: bool = False, action: Callable[[Any], None] = lambda x: None):
        if getattr(self._settings, key) == new_value:
            return False
